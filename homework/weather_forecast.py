...
class WeatherForecast:
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        # use torch.abs and torch.max and torch.mean and torch.sub
        day_means = torch.mean(self.data, dim=1, keepdim=True)
        idx = torch.argmax(torch.abs(self.data - day_means), dim=1)
        row_idx = torch.arange(self.data.shape[0])
        return self.data[row_idx, idx]
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        return torch.LongTensor(torch.argmin(diffs))
